    ...
    if(model_id != CAC_GAMERESOURCE_MODELID__ONLY_IF_MODDED)
    {
		if(0 <= model_id <= 19999) // valid GTA:SA game models are 0-19999
		{
			switch(component_type)
			{
				...
				{
					format(string, sizeof(string), "{FFFFFF}%s(%i) Game resource modded | Model ID: %d [DFF] | Checksum: %08x", playerName, player_id, model_id, checksum);
					if(model_id < 300 && model_id == GetPlayerSkin(player_id))
					{
						// Freeze player 
						//SetPVarInt(player_id, "frz", 1);
						//TogglePlayerControllable(player_id, false);
						SendClientMessage(player_id, 0xAFAFAFAA,
						"DFF model replacements are forbidden, select another skin! /skin <id>");
					}
				}
				case CAC_GAMERESOURCE_COMPONENTTYPE__TXD:
				{
					//format(string, sizeof(string), "SAMPCAC: {FFFFFF}Game resource modded: Player: %s [ID: %i] | Model ID: %d [TXD] | Checksum: %08x", playerName, player_id, model_id, checksum);
					//SendClientMessageToAll(0xEE5555FF, string);
				}
				case CAC_GAMERESOURCE_COMPONENTTYPE__ANIM:
				{
					format(string, sizeof(string), "{FFFFFF}%s(%i) Game resource modded | Model ID: %d [ANIM] | Checksum: %08x", playerName, player_id, model_id, checksum);
				}
				default:
				{
					format(string, sizeof(string), "{FFFFFF}%s(%i) Game resource modded | Model ID: %d [UNKNOWN] | Checksum: %08x", playerName, player_id, model_id, checksum);
				}
			}
		}
		else
		{
			...
			switch(model_id)
			{
				case CAC_GAMERESOURCE_MODELID__PEDIFP:
				{
					...
					// SetPVarInt(player_id,"CacModPED",model_id);
					new message[128];
					format(message, sizeof(message), "%s(%d) was kicked. Reason: Ped.ifp",
					playerName, player_id);
					SendClientMessageToAll(0xE10000AA, message);
					SetTimerEx("CAC_KickFix", 1000, false, "i", player_id);
				}
			}
			format(string, sizeof(string), "SAMPCAC: {FFFFFF}%s(%i) Game resource modded: %s | Checksum: %08x", playerName, player_id, modName, checksum);
		}
	}
	if(strlen(string) > 0) SendClientMessageToAll(0xEE5555FF, string);
